...
export class InMemoryOnePagerRepository implements OnePagerRepository, EmployeeRepository {
     ...
     */
    constructor(
        onePagers: Record<EmployeeID, { lastUpdateByEmployee: Date }[]>,
    ...
    async getAllOnePagersOfEmployee(employeeId: EmployeeID): Promise<OnePager[]> {
        this.logger.debug(
            `(InMemoryOnePagerRepository.ts: getAllOnePagersOfEmployee) Retrieving one-pagers for employee ${employeeId}.`,
        );
    ...
    async getAllEmployees(): Promise<EmployeeID[]> {
        this.logger.debug(
            '(InMemoryOnePagerRepository.ts: getAllEmployees) Returning all employees.',
        );
