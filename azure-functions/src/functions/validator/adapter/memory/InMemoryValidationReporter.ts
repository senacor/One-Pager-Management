...
export class InMemoryValidationReporter implements ValidationReporter {
    ...
    async reportErrors(id: EmployeeID, onePager: OnePager | undefined, errors: ValidationError[]): Promise<void> {
        this.logger.log(`(InMemoryValidationReporter.ts: reportErrors) Reporting the following errors for employee with id "${id}" and onePager ${JSON.stringify(onePager)}: ${JSON.stringify(errors)}`);
