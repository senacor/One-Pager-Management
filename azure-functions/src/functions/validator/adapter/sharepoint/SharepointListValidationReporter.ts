...
export class SharepointListValidationReporter implements ValidationReporter {
     ...
     */
    public static async getInstance(client: Client, siteAlias: string, listDisplayName: string, logger: Logger = console): Promise<SharepointListValidationReporter> {
        const maInfoSite = await client.api(`/sites/${siteAlias}`).get() as Site | undefined;
        if (!maInfoSite || !maInfoSite.id) {
            logger.error(`(SharepointListValidationReporter.ts: getInstance) Cannot find site with alias "${siteAlias}" !`);
        ...
        if (!lists) {
            logger.error(`(SharepointListValidationReporter.ts: getInstance) Cannot fetch lists for site with alias "${siteAlias}" !`);
        ...
        if (!listId) {
            logger.error(`(SharepointListValidationReporter.ts: getInstance) Cannot find list with name "${listDisplayName}" on site "${siteAlias}" !`);
