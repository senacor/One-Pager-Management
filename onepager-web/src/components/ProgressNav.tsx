                    ...
                    <ul>
                        {steps.map((step, index) => {
                            ...
                            return (
                                <li
                                    key={step.id}
                                    className={`relative pl-8 py-4 cursor-pointer text-sm font-medium transition-colors duration-300 hover:text-brand-blue ${isActive ? 'text-brand-blue' : 'text-white'
                                        } ${isCurrent ? 'font-semibold' : ''}`}
                                    onClick={() => onStepClick(index)}
                                >
                                    {/* Dot with completion indicator */}
                                    <div
                                        className={`absolute left-0 top-1/2 transform -translate-y-1/2 w-3 h-3 border-2 rounded-full transition-all duration-300 z-10 ${isCompleted
